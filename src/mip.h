...
class _mip_parser{
      ...
      private:
			  ...
			  hash_map <string, float, std_string_hash> Otu_conf_info;
...
int _mip_parser::Init(const char * infilename){
	...
	strin >> str_temp >> str_temp >> str_temp;
	...
	while(getline(infile, buffer)){
		...
		Otu_conf_info[a_otu] = a_otu_conf;
...
_mip _mip_parser:: Get_pathogen (hash_map<string, int, std_string_hash> otu_count, bool is_cp){
	...
	for (hash_map<string, int, std_string_hash>::iterator miter = otu_count.begin(); miter != otu_count.end(); miter ++){
		...
		if (Pathogen_otus.count(a_otu) != 0){ //a pathogen otu
			...
			pathogen_otu_conf += (float)a_otu_seq_count / cp_number * Otu_conf_info[a_otu];
	...
	if (pathogen_otu_abd > 0)
		a_mip.Pathogen_otu_conf = pathogen_otu_conf / pathogen_otu_abd;
...
void _mip_parser:: Output_pathogen_summary(const char * summary_file_name, vector <_mip> mips, vector <string> sample_names){
	...
	summary_out << "Sample_ID\tMiP\t#_of_OTU\t#_of_Pathogen_OTU\tConfidence" << endl;
	for (int i = 0; i < mips.size(); i ++)
		summary_out << sample_names[i] << "\t" << mips[i].Pathogen_otu_abd << "\t" << mips[i].Total_otu_count << "\t" << mips[i].Pathogen_otu_count << "\t" << mips[i].Pathogen_otu_conf << endl;
